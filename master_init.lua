...
term.setTextColor(colors.green)
io.write(string.format("%d/%d (%.1f%%)\n", response_success, NUMBER_OF_SLAVES, response_success/NUMBER_OF_SLAVES * 100))
term.setTextColor(colors.white)
io.write("SLAVE INIT TIMEOUT: ")
term.setTextColor(colors.yellow)
io.write(string.format("%d/%d (%.1f%%)\n", response_timeout, NUMBER_OF_SLAVES, response_timeout/NUMBER_OF_SLAVES * 100))
term.setTextColor(colors.white)
io.write("SLAVE INIT FAILURE: ")
term.setTextColor(colors.red)
io.write(string.format("%d/%d (%.1f%%)", response_failure, NUMBER_OF_SLAVES, response_failure/NUMBER_OF_SLAVES * 100))
term.setTextColor(colors.white)
print(string.rep("=", term_width))
