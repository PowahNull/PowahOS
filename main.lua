    ...
    PUSH = function(sender, body)
        ...
        if dest then
            ...
            for _, property in ipairs(candidates) do
                ...
                object_service.update(OBJECTS[source], OBJECTS[property.name])
        ...
        if source then
            ...
            for _, item in pairs(items) do
                ...
                for _, property in ipairs(candidates) do
                    ...
                    -- update sources
                    object_service.update(OBJECTS[dest], OBJECTS[property.name])
                end
...
function perform()
    while true do
        ...
        if content_ok then
            if switch[header] then
                internal_error = switch[header](sender, body) or false
            end
        end

        if internal_error then
            -- sort out internal error by performing full reload of data
            logger.fatal_error("exiting due to internal exception")
        end
    end
end
